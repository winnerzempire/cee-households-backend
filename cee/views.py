...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    # filterset_fields = ['featured', 'best_seller', 'new_arrival']  # Removed 'brand'
    lookup_field = 'slug'

    def get_queryset(self):
        queryset = Product.objects.filter(active=True).select_related(
            'category'  # Removed 'brand'
        ).prefetch_related(
        ...
        # 🏢 Brand filter removed since you don't have brand field
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        # Get shipping address from request data
        shipping_address_data = self.request.data.get('shipping_address', {})
        ...
        order = serializer.save(
            ...
            customer_phone=self.request.data.get('customer_phone', self.request.user.phone or ''),
            shipping_address=shipping_address_data,
            billing_address=self.request.data.get('billing_address', shipping_address_data)
        )
